...
--		The Admin Immunity Check code is from Jess-v.
--		The code updates to work with CS:GO Sourcebans bans & Different SteamID "Universes" is from Loo.
--
--		INSTRUCTIONS:
--		This module requires mysqloo! Get it here http://facepunch.com/showthread.php?t=1357773
--		Edit the config section below to your liking, make sure you add ulx_sban_servergbid to your server.cfg
...
local SBAN_WEBSITE			= "bans.example.com"	--Source Bans Website

local SBANDATABASE_HOSTNAME = "" 	//Database IP/Host
local SBANDATABASE_HOSTPORT = 3306				//Database Port
local SBANDATABASE_DATABASE = ""		//Database Database/Schema
local SBANDATABASE_USERNAME = ""		//Database Username
local SBANDATABASE_PASSWORD = ""		//Database Password
...
-- Table of Sourceban Groups and their ULX Group Alternative
-- This also needs to be corrected in /lua/ulx/modules/sh/sban_menu.lua
local sbToUlx = {
    -- Example: ["Sorucebans Group"] = <ulx group>
    -- IE: ["user"] = "user"
	["user"] = "user",
    ["admin"] = "admin",
...
-- ############### SteamID Universe Correction Functions #######
-- #############################################################
local function CorrectSteamIDToSteam_Zero(steamid) -- Gives STEAM_0
	local cut = string.Split(steamid, ":")
	
	if cut[1] == "STEAM_1" then
		cut[1] = string.Replace(cut[1], "1", "0")
		return table.concat(cut, ":")
	elseif cut[1] == "STEAM_0" then
		return table.concat(cut, ":")
	else
		return steamid
	end
...
local function CorrectSteamIDToSteam_One(steamid) -- Gives STEAM_1
	local cut = string.Split(steamid, ":")
	
	if cut[1] == "STEAM_1" then
		return table.concat(cut, ":")
	elseif cut[1] == "STEAM_0" then
		cut[1] = string.Replace(cut[1], "0", "1")
		return table.concat(cut, ":")
	else
		return steamid
	end
...
function SBAN_doban(inip, steamid, name, length, reason, callingadmin, lenderid)
	...
	query = query.." VALUES ('"..database_sban:escape(ip).."', '"..database_sban:escape(CorrectSteamIDToSteam_One(steamid)).."', '"..database_sban:escape(name).."',"..time..", "..(time + length)..", "..length..", '"..database_sban:escape(reason).."', "..adminid..", "..SBAN_SERVERID..");"
	SBAN_SQL_Query(query, qTab)
	...
	if lenderid and banLender then
		...
		query2 = query2.."VALUES ('"..database_sban:escape(ip).."', '"..database_sban:escape(CorrectSteamIDToSteam_One(lenderid)).."', '"..database_sban:escape(name).."',"..time..", "..(time + length)..", "..length..", '"..database_sban:escape(reason).."', "..adminid..", "..SBAN_SERVERID..");"
...
function SBAN_getadmin_id(steamid)
	...
	local qTab = {}
	qTab.wait = true
	local query = SBAN_SQL_Query("SELECT aid FROM "..SBAN_PREFIX.."admins WHERE authid in ('" ..CorrectSteamIDToSteam_One(steamid).. "', '"..CorrectSteamIDToSteam_Zero(steamid).."')", qTab)
	return query[1].aid
end
...
function SBAN_unban(steamid, ply, ureason)
	...
	local qTab = {}
	qTab.wait = false
	SBAN_SQL_Query("UPDATE "..SBAN_PREFIX.."bans SET RemovedOn = "..os.time()..", RemovedBy = "..adminID..", RemoveType = 'U', ureason = '"..database_sban:escape(ureason).."' WHERE authid = '"..database_sban:escape(CorrectSteamIDToSteam_One(steamid)).."' and RemoveType is null", qTab)
	XGUIRefreshBans()
end
...
function SBAN_canunban(steamid, ply)
	...
	local query = "SELECT * FROM "..SBAN_PREFIX.."bans WHERE authid = '"..database_sban:escape(CorrectSteamIDToSteam_One(steamid)).."' and RemoveType is null"
...
function SBAN_updateban(steamid, ply, bantime, reason, name)
	...
	query = query .. ", length = "..bantime..", name = '"..database_sban:escape(updateName).."', reason = '"..database_sban:escape(reason).."' WHERE authid = '"..database_sban:escape(CorrectSteamIDToSteam_One(steamid)).."' and RemoveType is null"
	SBAN_SQL_Query(query, qTab)
	XGUIRefreshBans()
end
-- ############### Unban UI  Section ###############
...
local function CheckAdmin(result, qTab, lev)
	...
	elseif lev == "checkadmingroup" then
		...
		if #result >= 1 then --Admin has permissions for this specific server, finding admin group to add.
			...
			SBAN_SQL_Query("SELECT srv_group FROM "..SBAN_PREFIX.."admins WHERE authid in ('" ..CorrectSteamIDToSteam_Zero(steamid).. "', '"..CorrectSteamIDToSteam_One(steamid).."')", qTab)
			return
		else
	...
	elseif lev == "addtogroup" then
		...
		if group != nil and string.len(group) > 0 then
			...
			if(ULib.ucl.getUserRegisteredID(ply) == nil || ply:GetUserGroup() != sbToUlx[group]) && (excludedGroups[ply:GetUserGroup()] == nil ) then
				
				ulx.adduserid(ply, steamid, sbToUlx[group])
			end
			return
		else
			RemoveAdmin(ply)
...
local function StartAdminCheck(ply, steamid)
	...
	SBAN_SQL_Query("SELECT aid FROM "..SBAN_PREFIX.."admins WHERE authid in ('" ..CorrectSteamIDToSteam_Zero(steamid).. "', '"..CorrectSteamIDToSteam_One(steamid).."')", qTab)
end
...
local function StartBanCheck(ply, steamid)
	...
	SBAN_SQL_Query("SELECT bid, authid, ends, length, reason, RemoveType FROM "..SBAN_PREFIX.."bans WHERE authid = '" ..CorrectSteamIDToSteam_One(steamid).. "'", qTab)
	...
	ip = string.Split(ply:IPAddress(), ':')[1] or nil
	if ip then
		SBAN_SQL_Query("SELECT bid, authid, ends, length, reason, RemoveType FROM "..SBAN_PREFIX.."bans WHERE ip = '" ..ip.. "'", qTab)
	end
end
...
hook.Add("TTTKarmaLow", "KarmaSourceBan", function(ply)
	-- StartBanCheck(ply, ply:SteamID())
	if tttKarmaBan and KARMA and KARMA.cv.enabled:GetBool() and KARMA.cv.autoban:GetBool() then
		SBAN_doban(ply:IPAddress(),ply:SteamID(), ply:Nick(), 60*60, "Karma too low", 0, ply.lenderid)
	end
end)
