...
local rankTable = {
    -- Example: ["ulx group"] = <Immunity Level>
    -- IE: ["user"] = 1
	["user"] = 1, -- 0
    ["admin"] = 2,
    ["superadmin"] = 3
...
function ulx.sbanid( calling_ply, steamid, minutes, reason )
	...
	local callPlyData = ULib.ucl.getUserInfoFromID(calling_ply:SteamID())
	local targetPlyData = ULib.ucl.getUserInfoFromID(steamid)
	if not targetPlyData then
		targetGroup = "user"
	else
		targetGroup = tostring(targetPlyData.group)
		targetGroup = targetGroup:lower()
	end

	local callGroup = tostring(callPlyData.group)
	callGroup = callGroup:lower()
	local name
	local targetPly = player.GetBySteamID(steamid)
	name = targetPly and targetPly:Nick() or nil
	if rankTable[targetGroup] == nil then
		targetGroup = "user"
	end
	if rankTable[callGroup] == nil then
		targetGroup = "user"
	end
	if rankTable[callGroup] >= rankTable[sbToUlx[targetGroup]] then
		local time = "for #i minute(s)"
		if minutes == 0 then time = "permanently" end
		...
		ULib.tsayError( calling_ply, "You cannot ban a rank higher than you." )
		return
	end
end
...
function ulx.unsban( calling_ply, steamid, ureason )
	...
	if SBanTable[ steamid ] then
		if ( SBanTable[ steamid ].adminid ~= calling_ply.sb_aid ) and not ULib.ucl.query( calling_ply, "ulx unsbanall" )  then
			ULib.tsayError( calling_ply, "Error: You do not have permission to remove another admin's ban.1", true )
			return
		end
	elseif not SBAN_canunban(steamid, calling_ply) then
		ULib.tsayError( calling_ply, "Error: You do not have permission to remove another admin's ban.2", true )
		return
	end
